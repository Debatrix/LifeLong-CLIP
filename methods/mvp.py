...
class MVP(_Trainer):
    ...
    def online_train(self, data):
        ...
        if len(self.memory) > 0 and self.memory_batchsize > 0:
            ...
            #     memory_labels[i] = self.exposed_classes.index(
    ...
    def _compute_grads(self, feature, y, mask):
        ...
        total_batch_grad = head.weight.grad[:len(self.exposed_classes)].clone(
        )  # C,dim
    ...
    def _get_compensation(self, y, feat):
        ...
        cps_score = (1. - torch.cosine_similarity(head_w, feat, dim=1) +
                     self.margin)  #B
        return cps_score
    ...
    def loss_fn(self, feature, mask, y):
        ...
        if self.use_afs:
            logit = self.model_without_ddp.forward_head(feature)
            logit = self.model_without_ddp.forward_head(
                feature / (cps_score.unsqueeze(1)))
        else:
            logit = self.model_without_ddp.forward_head(feature)
        if self.use_mask:
            logit = logit * mask
